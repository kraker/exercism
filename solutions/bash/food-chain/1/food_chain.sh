...
build_stanza() {
  ...
  if ((n > 1)); then
    ...
    for ((i = n; i >= 2; i--)); do
      if ((i == 3)); then
        lines+=("She swallowed the ${food[$i]} to catch the ${food[$i - 1]} that wriggled and jiggled and tickled inside her.")
      else
        lines+=("She swallowed the ${food[$i]} to catch the ${food[$i - 1]}.")
      fi
    done
  fi
